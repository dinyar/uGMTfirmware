...
def parse_options():
    ...
    parser.add_argument('lut_types', type=str, nargs='+', default='binary', help='LUT-types you want to checkout.')
...
def main():
    ...
    for lut_type in options.lut_types:
        # get the addresses for download
        addresses = get_addresses(lut_type)
        # check if sub-folder exists:
        folder_name = os.path.join(options.outpath, lut_type)
